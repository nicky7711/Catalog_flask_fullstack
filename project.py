...
# Connect to Google OAuth, validate it, then get user information
...
# Export menu information to JSON
...
# Export restaurant information to JSON
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
        flash('You are not authorized to edit %s' & editedRestaurant.name)
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        flash('You are not authorized to delete %s' & restaurantToDelete.name)
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        flash('You are not authorized to add %s' & restaurant.name)
